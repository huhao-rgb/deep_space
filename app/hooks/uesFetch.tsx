...
export type UA = false | 'pc' | 'mobile'

const postContentType = (isPost: boolean) => {
  return isPost
...
const chooseUserAgent = (ua: UA = false) => {
  const userAgent = {
    mobile: [
  ...
  const { mobile, pc } = userAgent
  const realUserAgentList = ua === false
    ? [...mobile, ...pc]
    : userAgent[ua]
...
export const useFetch = (options: FetchOptions) => {
  ...
  const customHeaders = {
    ...headers,
    ...postContentType(method === 'POST'),
    ...referer(url.includes('music.163.com')),
